...
import { AssignmentResponseDto, AssignmentAttachmentDto } from '../../models/assignment.model';
...
export class AssignmentService {
  ...
  private normalizeAssignment(u: any): AssignmentResponseDto {
    return {
      ...
      assignmentAttachments: (u.assignmentAttachments ?? u.AssignmentAttachments ?? [])
        .map((a: any) => this.normalizeAttachment(a)),
    };
  }

  getAssignmentsByCourseId(courseId: number): Observable<AssignmentResponseDto[]> {
    return this.http.get<any[]>(`${this.apiUrl}/course/${courseId}`)
      .pipe(map(list => list.map(u => this.normalizeAssignment(u))));
  }

  createOrUpdateAssignment(courseId: number, data: { id: number, title: string, description: string, dueDate: string, totalMarks: number }): Observable<AssignmentResponseDto> {
